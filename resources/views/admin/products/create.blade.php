            ...
            <div class="p-6 bg-bg-soft border-b border-border">
                    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name') }}" class="mt-1 block w-full rounded-md border-border shadow-sm focus:border-brand-300 focus:outline-none @error('name') border-red-500 @enderror" required>
                            @error('name')
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4" class="mt-1 block w-full rounded-md border-border shadow-sm focus:border-brand-300 focus:outline-none @error('description') border-red-500 @enderror" required>{{ old('description') }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" id="price" name="price" value="{{ old('price') }}" step="0.01" min="0" class="mt-1 block w-full rounded-md border-border shadow-sm focus:border-brand-300 focus:outline-none @error('price') border-red-500 @enderror" required>
                            @error('price')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex justify-end space-x-4 pt-4">
                            ...
                            <button type="submit" class="px-4 py-2 bg-brand-300 text-bg-soft dark:bg-brand-200 dark:text-bg-main rounded-md hover:bg-brand-400 dark:hover:bg-brand-100 transition">
                                {{ __('messages.buttons.create_product') }}
                            </button>
                        </div>
                    </form>
                </div>
