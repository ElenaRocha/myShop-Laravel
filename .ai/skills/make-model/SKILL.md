...
0. **Comprobación previa**: si `app/Models/{Modelo}.php` ya existe, muéstralo y pregunta antes de sobrescribir (puede tener accessors, scopes o lógica añadida a mano que no debe perderse).

1. **Deduce la configuración del modelo**:
...
2. **Infiere las relaciones desde las claves foráneas** (lee también la migración del otro extremo para el lado inverso):
...
3. Sigue las convenciones del proyecto (PHPDoc de retorno, orden de imports): revisa un modelo existente como referencia.
